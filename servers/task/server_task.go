...
func server(param interface{}) (result bool) {
	...
	currentTimeTounix := uint64(currentTime.Unix())
	fmt.Println("定时任务，服务注册", param, server, currentTime.Format(time.Layout))

	cache.SetServerInfo(server, currentTimeTounix)

...
func (manager *ClientManager) GetClients() (clients map[*Client]bool) {
	...
	// 超时的话直接从Clients中删除用户 不是设置v为false
...
func ClearTimeoutConnections() {
	...
	overtimeUser := make([]string, 0)

	for client, _ := range clients {
		if client.IsHeartbeatTimeout(currentTime) {
			fmt.Println("心跳时间超时 关闭连接", client.Addr, client.UserId, client.LoginTime, client.HeartbeatTime)
			userKey := strconv.Itoa(int(client.AppId)) + "_" + client.UserId
			key := cache.GetUserOnlineKey(userKey)
			overtimeUser = append(overtimeUser, key)
			client.Socket.Close()
		}
	}

	err := cache.DelOvertimeUsersInRedis(overtimeUser)
	if err != nil {
		fmt.Println("ClearTimeoutConnections Error ", err.Error())
	}
